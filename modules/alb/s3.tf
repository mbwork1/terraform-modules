...
resource "aws_s3_bucket" "lb_logs_togab" {
  count  = local.create_logging_bucket ? 1 : 0
...
resource "aws_s3_bucket_server_side_encryption_configuration" "lb_logs_togab" {
  count  = local.create_logging_bucket ? 1 : 0
...
resource "aws_s3_bucket_versioning" "lb_logs_togab" {
  count  = local.create_logging_bucket ? 1 : 0
  bucket = aws_s3_bucket.lb_logs_togab[0].id
...
resource "aws_s3_bucket_policy" "lb_logs_togab" {
  count  = local.create_logging_bucket ? 1 : 0
  bucket = aws_s3_bucket.lb_logs_togab[0].id
  policy = <<EOF
{
